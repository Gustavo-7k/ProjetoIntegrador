...
// Verificar tamanho (10MB max para capas)
$maxSize = 10 * 1024 * 1024;
if ($file['size'] > $maxSize) {
    sendJSONResponse(['ok' => false, 'error' => 'Arquivo muito grande. Máximo: 10MB'], 422);
}
...
try {
    $pdo = getDBConnection();
    $stmt = $pdo->prepare('SELECT cover_image FROM users WHERE id = ?');
    ...
    error_log('Erro ao atualizar cover_image: ' . $e->getMessage());
    // Arquivo foi salvo, mas falhou no banco - tentar remover
    @unlink($destPath);
    sendJSONResponse(['ok' => false, 'error' => 'Erro ao salvar no banco de dados'], 500);
}

// Retornar caminho relativo para uso no frontend
